    ...
    <footer className="bg-card/20 border-t border-border">
      <div className="container mx-auto px-6 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {/* Company Info */}
          <div className={`md:col-span-2 ${language === "ar" ? "text-right" : "text-left"}`}>
          ...
          {/* Contact Info */}
          <div className={`${language === "ar" ? "text-right" : "text-left"}`}>
            <h4 className="text-white font-semibold mb-4">{language === "ar" ? "تواصل معنا" : "Contact Info"}</h4>
            <ul className="space-y-3">
              <li className={`flex items-center gap-3 text-white/70 ${language === "ar" ? "flex-row-reverse" : ""}`}>
                <Phone size={16} />
              ...
              <li className={`flex items-center gap-3 text-white/70 ${language === "ar" ? "flex-row-reverse" : ""}`}>
                <MapPin size={16} />
                <span>{language === "ar" ? "ليبيا، طرابلس" : "Tripoli, Libya"}</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        {/* Bottom Bar */}
        <div
          className={`mt-12 pt-8 border-t border-border flex flex-col md:flex-row justify-between items-center gap-4 ${language === "ar" ? "md:flex-row-reverse" : ""}`}
        >
