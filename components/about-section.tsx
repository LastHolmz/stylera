...
export default function AboutSection() {
  ...
  const values = [
    ...
    {
      icon: Users,
      titleKey: language === "ar" ? "التعاون والشراكة" : "Collaboration & Partnership",
      descriptionKey:
        language === "ar"
    ...
    {
      icon: Lightbulb,
      titleKey: language === "ar" ? "الابتكار والإبداع" : "Innovation & Creativity",
      descriptionKey:
        language === "ar"
    ...
    {
      icon: Award,
      titleKey: language === "ar" ? "التميز والاحترافية" : "Excellence & Professionalism",
      descriptionKey:
        language === "ar"
  ...
  return (
    <section id="about" className="py-24 relative bg-card/20">
      <div className="container mx-auto px-6">
        {/* Section Header */}
        <div className={`max-w-4xl mx-auto mb-20 ${language === "ar" ? "text-right" : "text-left"}`}>
          <h2 className="text-4xl md:text-5xl font-light text-white mb-8">
            <span className="font-medium instrument text-accent">{t("about.title")}</span>
          </h2>
          <p className="text-xl md:text-2xl text-white/80 font-light leading-relaxed mb-8">{t("about.description")}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
          <div
            className={`bg-gradient-to-br from-primary/10 to-accent/10 backdrop-blur-sm border border-border rounded-2xl p-8 ${language === "ar" ? "text-right" : "text-left"}`}
          >
            <h3 className="text-2xl font-semibold text-white mb-6">{language === "ar" ? "رؤيتنا" : "Our Vision"}</h3>
          ...
          <div
            className={`bg-gradient-to-br from-accent/10 to-primary/10 backdrop-blur-sm border border-border rounded-2xl p-8 ${language === "ar" ? "text-right" : "text-left"}`}
          >
            <h3 className="text-2xl font-semibold text-white mb-6">{language === "ar" ? "مهمتنا" : "Our Mission"}</h3>
            <p className="text-white/80 font-light leading-relaxed">
              {language === "ar"
        ...
        {/* Values */}
        <div className={`mb-20 ${language === "ar" ? "text-right" : "text-left"}`}>
            ...
            {values.map((value, index) => {
              ...
              return (
                <div key={index} className="group text-center hover:scale-105 transition-transform duration-300">
                  ...
                  <h4 className="text-lg font-semibold text-white mb-3">{value.titleKey}</h4>
                  <p className="text-white/70 font-light leading-relaxed text-sm">{value.descriptionKey}</p>
                </div>
              )
            })}
          </div>
        ...
        <div className="bg-gradient-to-r from-primary/5 to-accent/5 backdrop-blur-sm border border-border rounded-2xl p-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div>
              ...
              <div className="text-white/70 font-light">{language === "ar" ? "مشروع مكتمل" : "Projects Completed"}</div>
            </div>
            <div>
              <div className="text-4xl md:text-5xl font-bold text-white mb-2">25+</div>
              <div className="text-white/70 font-light">{language === "ar" ? "عميل سعيد" : "Happy Clients"}</div>
            </div>
            <div>
              <div className="text-4xl md:text-5xl font-bold text-white mb-2">5+</div>
              <div className="text-white/70 font-light">{language === "ar" ? "سنوات خبرة" : "Years Experience"}</div>
            </div>
            <div>
