...
export default function HeroContent() {
  ...
  return (
    <main className={`absolute bottom-8 z-20 max-w-2xl px-6 ${language === "ar" ? "right-8" : "left-8"}`}>
      <div className={`${language === "ar" ? "text-right" : "text-left"}`}>
        ...
        <h1 className="text-5xl md:text-7xl md:leading-tight tracking-tight font-light text-white mb-4">
          <span className="font-medium italic instrument text-accent">{t("hero.title")}</span>
          <br />
          <span className="font-light tracking-tight text-white/90 text-3xl md:text-4xl">{t("hero.subtitle")}</span>
        </h1>

        {/* Description */}
        <p className="text-base font-light text-white/70 mb-8 leading-relaxed max-w-lg">{t("hero.description")}</p>

        {/* Buttons */}
        <div className={`flex items-center gap-4 flex-wrap ${language === "ar" ? "rtl:flex-row-reverse" : ""}`}>
