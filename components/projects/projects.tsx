...
export default function AllProjects({
  ...
  const filteredProjects = allProjects.filter((p) => {
    const matchesSearch =
      p[locale].title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      p[locale].description.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesCategory =
      selectedCategory === "all" ||
      !selectedCategory ||
      p[locale].category === selectedCategory;
          ...
          >
            ...
            <Select onValueChange={(val) => setSelectedCategory(val)}>
              ...
              <SelectContent className="bg-slate-800 text-slate-200 border-slate-700 font-[cairo]">
                ...
                {Array.from(
                  new Set(allProjects.map((p) => p[locale].category))
                ).map((category) => (
          ...
          {filteredProjects.length > 0 ? (
            filteredProjects.map((p, index) => (
              <ProjectCard
                ...
                image={p[locale].image}
                category={p[locale].category}
                technologies={p[locale].technologies}
                github={p[locale].github}
                live={p[locale].live}
              />
            ))
          ) : (
